...
<nav>
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('actors.view', ['staticURL' => $staticURL]) }}">View</a></li>
    </ol>
</nav>
@endsection
...
<div class="card">
    <div class="card-body">
        ...
        <form class="row g-3 needs-validation mt-2" novalidate method="post" action="{{ route('actors.update', ['staticURL' => $staticURL]) }}">
            @csrf
            @method('PATCH')
            <div class="col-md-6">
                <label for="actorName" class="form-label">Actor Name</label>
                <input type="text" class="form-control" name="actorName" id="actorName" value="{{ old('actorName', $actorName) }}" required>
                <div class="invalid-feedback">
                    Required
                </div>
                @if ($errors->has('actorName'))
                <p class="text-danger">{{ $errors->first('actorName') }}</p>
                @endif
            </div>
            <div class="col-md-6">
                <label for="staticURL" class="form-label">Static URL</label>
                <input type="text" class="form-control" name="staticURL" id="staticURL" value="{{ old('staticURL', $staticURL) }}" required>
                <div class="invalid-feedback">
                    Required
                </div>
            ...
            <div class="col-12">
                ...
                <a class="btn btn-link" type="button" id="backButton" href="{{ route('actors.page') }}">Back</a>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#actorName').on('input', function() {
            let actorName = $(this).val();
            let staticURL = toSlug(actorName);

...
<div class="row">
  ...
  <div class="col-lg-12">
    <div class="card">
      <div class="card-body">
        <a type="button" class="btn btn-primary mt-4 mb-4" href="{{ route('actors.add') }}">
          <i class="bi bi-plus-circle"></i>
          <span>Add Actor</span>
        </a>
        <!-- Table with stripped rows -->
        <div id="actorsListHolder"></div>
...
<script>
  $(document).ready(function() {
    callAPI(
      ...
      function(data) {
        ...
        if (responseData.actors.length == 0) {
          $('#actorsListHolder').empty().append(`
          ...
          $('#actorsListHolder').empty().append(`
            ...
            `)
          for (let index = 0; index < responseData.actors.length; index++) {
            const element = responseData.actors[index];
            let routeView = "{{ route('actors.view', ['staticURL' => '__STATIC_URL__']) }}"
            let routeDelete = "{{ route('actors.delete', ['staticURL' => '__STATIC_URL__']) }}"
                        ...
                        <div class="modal-dialog">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title">Actor Deleting</h5>
                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                              Are you sure to delete ${element.actorName} actor?
                            </div>
                            <div class="modal-footer">
                              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" id="${element.staticURL}CloseButton">Close</button>
                              <button type="button" class="btn btn-danger" onclick=deleteActor("${element.staticURL}","${routeDelete}") id="${element.staticURL}DeleteButton">Delete</button>
                            </div>
                          </div>
                        </div>
  ...
  function deleteActor(staticURL, endpoint) {
    callAPI(
      'delete',
      endpoint,
      function() {

...
<nav>
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('genres.view', ['staticURL' => $staticURL]) }}">View</a></li>
    </ol>
</nav>
@endsection
...
<div class="card">
    <div class="card-body">
        ...
        <form class="row g-3 needs-validation mt-2" novalidate method="post" action="{{ route('genres.update', ['staticURL' => $staticURL]) }}">
            @csrf
            @method('PATCH')
            <div class="col-md-6">
                <label for="genreName" class="form-label">Genre Name</label>
                <input type="text" class="form-control" name="genreName" id="genreName" value="{{ old('genreName', $genreName) }}" required>
                <div class="invalid-feedback">
                    Required
                </div>
                @if ($errors->has('genreName'))
                <p class="text-danger">{{ $errors->first('genreName') }}</p>
                @endif
            </div>
            <div class="col-md-6">
            ...
            <div class="col-12">
                ...
                <a class="btn btn-link" type="button" id="backButton" href="{{ route('genres.page') }}">Back</a>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#genreName').on('input', function() {
            ...
            let staticURL = toSlug(genreName);
